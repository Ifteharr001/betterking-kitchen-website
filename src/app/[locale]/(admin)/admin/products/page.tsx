...
const AdminProducts = () => {
  ...
  const fetchData = async (pageNumber = 1) => {
    try {
      ...
      const [prodRes, catRes] = await Promise.all([
        fetch(`/api/admin/products?page=${pageNumber}&limit=${limit}`, { cache: "no-store" }),
      ...
      const productsResponse = Array.isArray(prodData) ? prodData : prodData.products || [];
      const totalResponse = Array.isArray(prodData) ? prodData.length : prodData.total || 0;
      const pageResponse = Array.isArray(prodData) ? pageNumber : prodData.page || pageNumber;
  ...
  const getCategoryName = (catId: string) => {
    ...
    return cat ? getEnText(cat.name) : catId;
  };
 ...
 const getSubCategoryName = (catId: string, subId?: string) => {
    ...
    if (cat && cat.subCategories) {
      ...
      return sub ? getEnText(sub.name) : "Unknown Sub-category";
    }
      ...
      <Card className="bg-white border-gray-200 shadow-sm">
        <CardHeader className="pb-3 border-b border-gray-100">
          <CardTitle className="text-base font-semibold flex items-center gap-2 text-gray-800">
              <Package className="w-4 h-4 text-primary" /> All Products ({total})
          </CardTitle>
        </CardHeader>
            ...
            <>
                    ...
                    <TableRow key={p.id} className="hover:bg-gray-50/50 border-b border-gray-100/50">
                      ...
                      <TableCell>
                        <div className="flex flex-col">
                          <span className="font-semibold text-gray-900">{getEnText(p.name)}</span>
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <Link href={`/${locale}/admin/products/edit/${p.id}`}>
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  This will permanently delete <span className="font-bold text-gray-900">"{getEnText(p.name)}"</span> from the database. This action cannot be undone.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 py-3 border-t border-gray-100 bg-white">
                <div className="text-sm text-gray-600">
                  Showing {Math.min((page - 1) * limit + 1, total)}&ndash;{Math.min(page * limit, total)} of {total} products
                </div>
