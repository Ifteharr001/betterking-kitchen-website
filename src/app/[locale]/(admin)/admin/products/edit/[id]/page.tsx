  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setFormData({
          id: productData.id,
          name: getEnText(productData.name),
          category: productData.category || "",
          subCategory: productData.subCategory || "",
          description: getEnText(productData.description),
        });
        
        setImage(productData.image);
        setGallery(productData.gallery || []);

        const englishFeatures = productData.features?.length > 0 
          ? productData.features.map((f: any) => getEnText(f)) 
          : [""];
        setFeatures(englishFeatures);

        const englishHighlights = productData.highlights?.length > 0 
          ? productData.highlights.map((h: any) => ({ icon: h.icon, text: getEnText(h.text) })) 
          : [{ icon: "", text: "" }];
        setHighlights(englishHighlights);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="space-y-1.5">
              ...
              >
                ...
                <SelectContent className="bg-[#0c1727] border-gray-200 z-50">
                  {categoriesList.map((cat) => (
                    <SelectItem key={cat._id} value={cat._id}>{getEnText(cat.name)}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
              ...
              >
                ...
                <SelectContent className="bg-[#0c1727] border-gray-200 z-50">
                  {subCategoriesList.map((sub) => (
                    <SelectItem key={sub._id} value={sub._id}>{getEnText(sub.name)}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
