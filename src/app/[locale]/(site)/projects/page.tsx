    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main>
        ...
        <section className="bg-white">
          <div className="container mx-auto px-4 lg:px-8 py-16 md:py-24">
            <div className="text-center mb-12">
              <span className="text-xs font-semibold uppercase tracking-wider text-primary">{t("spotlightBadge")}</span>
              <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold mt-2 mb-3 text-gray-900">{t("spotlightTitle")}</h2>
                    ...
                    <div className={i % 2 === 1 ? "md:order-1" : ""}>
                      <div className="flex items-center gap-3 mb-3">
                        <span className="text-[11px] font-semibold uppercase tracking-wider bg-primary/10 text-primary px-3 py-1 rounded-full">
                          {getLocalizedText(blog.category, locale)} 
                        </span>
                      ...
                      <p className="text-sm text-gray-400 flex items-center gap-1 mb-4">
                        ...
                        {getLocalizedText(blog.location, locale) || "Global"} 
                      </p>
                      <p className="text-sm md:text-base text-gray-600 leading-relaxed mb-5 line-clamp-3">
                        {getLocalizedText(blog.shortDesc, locale)}
                      </p>
                      <Link href={`/${locale}/news/${getLocalizedText(blog.slug, locale) || blog.slug}`}>
        ...
        <section className="bg-white">
          <div className="container mx-auto px-4 lg:px-8 py-16 md:py-24">
            <div className="text-center mb-10">
              <span className="badge-gold mb-4 inline-block">{t("galleryBadge")}</span>
              <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-3 text-gray-900">{t("galleryTitle")}</h2>
              <p className="text-gray-500 text-sm md:text-base max-w-lg mx-auto">{t("galleryDesc")}</p>
            </div>
                    ...
                    <div className="group rounded-xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md transition-all cursor-pointer relative h-48 md:h-56 w-full">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
                        ...
                        <p className="text-white/70 text-xs line-clamp-1">
                          {getLocalizedText(p.description, locale)}
                        </p>
                      </div>
                    </div>
                  </Link>
        ...
        <section className="container mx-auto px-4 lg:px-8 py-16 md:py-24">
          <div className="text-center mb-10">
            <span className="badge-gold mb-4 inline-block">{t("testBadge")}</span>
            <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-3">{t("testTitle1")} <span className="gradient-headline">{t("testTitle2")}</span></h2>
            <p className="text-muted-foreground text-sm md:text-base max-w-lg mx-auto">{t("testDesc")}</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-5">
            {testimonials.map((tItem, i) => (
              <div key={i} className="feature-card p-6 md:p-8 flex flex-col">
                ...
                <p className="text-sm md:text-base text-muted-foreground leading-relaxed flex-1 mb-6">"{tItem.quote}"</p>
                <div>
            ...
            <div className="relative rounded-2xl overflow-hidden p-8 md:p-14 text-center bg-gray-900">
              ...
              <div className="relative z-10">
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                  <Link href={`/${locale}/contact`} className="btn-gold gap-2">
