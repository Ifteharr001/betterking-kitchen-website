...
export type SubCategory = { _id: string; name: string; description: string; image: string; categoryId: string; };
export type Category = { _id: string; name: string; description: string; image: string; subCategories: SubCategory[]; };
            ...
            <AccordionItem key={cat._id} value={cat._id} className="border border-border rounded-lg overflow-hidden bg-card">
              <AccordionTrigger className="px-5 py-4 hover:no-underline">
                <div className="flex items-center gap-4 flex-1 text-left">
                  <img src={cat.image} alt={cat.name} className="w-12 h-12 rounded-lg object-cover border border-border flex-shrink-0" />
                    ...
                    <div className="grid gap-2">
                      {cat.subCategories?.map((sub) => (
                        <div key={sub._id} className="flex items-center gap-3 p-3 rounded-lg bg-secondary/50 border border-border/50">
                          <img src={sub.image} alt={sub.name} className="w-10 h-10 rounded-md object-cover border border-border flex-shrink-0" />
                          <div className="flex-1 min-w-0">
